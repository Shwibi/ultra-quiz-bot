...
class Event extends Index.EntryPoint {
	...
	async init(client) {
		...
		for (const folder of commandFolders) {
			...
			for (const file of commandFiles) {
				...
				this.cache.push(
					new Cache(
						`Loaded command "${command.name}" in category "${folder}".`
					).Log()
				);
			}
		}
	...
	async call(message) {
		if (!this.initiated)
			return new Err(`Called ${EventName} event without initialisation!`);
		...
		if (message.guild && !global.prefixes.get(message.guild.id)) {
			message.guildDB = await Guilds.findOne({
			...
			message.prefix =
				(await message.guildDB.get("prefix")) || this.config.Bot.prefix;
		...
		// Canary
		message.prefix = this.config.Bot.prefix;

		// Help by ping
		if (msg.includes(this.client.user.id))
			message.reply(
				`Prefix is: \`${message.prefix}\`. You can use \`${message.prefix}help\` to look at help! :)`
			);
		...
		const res =
			...
			this.client.commands.find((c) => c.aliases && c.aliases.includes(req));
