...
class Command extends Message.Event {
  ...
  call(message) {
    ...
    if(!type || !toLog) return message.reply(`Please use the valid format! \`<prefix>log <type:dev/build/v13> <To Log>\``);
    ...
    additions.split(":").slice(1).forEach(add => {
    ...
    const logChannel = this.client.channels.cache.get(this.config.Dev.logs_channel);
    if(!logChannel) return message.reply(`No log channel found!`);
