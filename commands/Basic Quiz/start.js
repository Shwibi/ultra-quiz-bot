...
class Command extends Message.Event {
                  ...
                  for (let lbgGB = 0; lbgGB < maxGB; lbgGB++) {
                    ...
                    if (user)
                      guildBoardEmbed.addField(
                        `Rank ${lbgGB + 1}. ${user.username} | ${user.id}`,
                        `<@${userId}> Correct: ${
  ...
  async askQuestion(
    ...
    let GetQuizRunning = await QuizRunning.findOne({ messageId: message.id });
    const q = qd[i];
    if (!GetQuizRunning)
      GetQuizRunning = await QuizRunning.create({
        quizId: quizId,
        ...
        answeredUsers: [],
        crossboard: globalBoard,
      });
    else
      await GetQuizRunning.updateOne({
        questionNumber: i,
        timeRemaining: q.time,
      });
    const timeRem = timeRemaining ? timeRemaining : q.time;
    const QuestionEmbed = new Discord.MessageEmbed()
      .setDescription(`Click on the button that you think is the best answer!`)
      .setColor("RED")
      .setFooter(`You have ${timeRem / 1000 || 30} seconds.`);
    if (q.question.length > 230) {
      QuestionEmbed.setTitle(
        `${i + 1}. (See description for question)`
      ).setDescription(q.question);
    } else QuestionEmbed.setTitle(`${parseInt(i) + 1}. ${q.question}`);
    const optVals = {};
    ...
    message.channel.send(QuestionEmbed, optButtonsRow).then((embedMsg) => {
      ...
      let localLeaderboard = collectedUsers ?? [];
      ...
      this.client.on("clickButton", async (button) => {
        ...
        if (buttonArgs[0] == quizId && buttonArgs[1] == i + 1 && quizRunning) {
          ...
          if (answeredUsers.length > 0)
            await GetQuizRunning.updateOne({ answeredUsers: answeredUsers });
          if (buttonArgs[2] == correctIndex) {
            ...
            await GetQuizRunning.updateOne({
              collectedUsers: localLeaderboard,
            });
          } else {
      ...
      shwijs.Countdown(
        timeRem / 1000 || 30,
        async (err, timeElapsed, timeRemaining) => {
          if (err) return err.log();
          ...
          if (t % 60 == 0) {
            await GetQuizRunning.updateOne({ timeRemaining: t * 1000 });
          }
        },
        async () => {
          quizRunning = false;
          ...
          await GetQuizRunning.updateOne({ crossboard: globalBoard });
          embedMsg.edit(
          ...
          if (qd[i + 1] && !this.needToStop[message.channel.id]) {
            ...
            await GetQuizRunning.updateOne({ completed: true });
            callbackOnEnd(globalBoard);
          }
        }
      );

      // const collector = new Discord.MessageCollector(message.channel, msg => !msg.author.bot, { time: timeRem || 30000 });
