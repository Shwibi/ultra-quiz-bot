...
class Command extends Message.Event {
	...
	async call(message) {
		...
		if (!quizId)
			return message.channel.send(`Please provide a quiz id to delete!`);
		if (isNaN(parseInt(quizId)))
			return message.channel.send(`Please provide a valid quiz id to delete!`);
		if (this.deleted.includes(quizId))
			return message.channel.send(this.notExistMsg);

		// Search for quiz
		if (this.doesNotExist.includes(quizId))
			return message.channel.send(this.notExistMsg);
		...
		if (quizDb.creator == message.author.id || message.isDev) {
			quizDb.deleteOne().then(message.channel.send(`Deleted quiz ${quizId}!`));
			Start.instance.deleteQuiz(quizId);
			Search.instance.delete(quizId);
			View.instance.delete(quizId);
		} else {
			message.channel.send(
				`You do not have the permission to delete that quiz!`
			);
		}
	}
}
