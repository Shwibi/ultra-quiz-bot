...
class Command extends Message.Event {
  ...
  async call(message) {
    ...
    if (foundInCache) {
      ...
      const findQuizzesFromDB = [...(await QuizModel.find({
        name: {
      ...
      const QuizFoundEmbed = new Discord.MessageEmbed()
        .setTitle(`Found ${findQuizzesFromDB.length} quiz(zes)! | ${firstQuiz.name}`)
        .setDescription(`Name: ${firstQuiz.name} | Total questions: ${firstQuiz.quizDetails.length} | ID: ${firstQuiz.quizId}`)

        .setColor("RANDOM");
      if (idArray.length > 0) {
        QuizFoundEmbed.addField(`Other quiz(zes) with similar name`, idArray.join(", \n"));
      }
