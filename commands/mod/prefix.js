...
class Command extends Message.Event {
	...
	async call(message) {
		...
		if (!args[1])
			return message.reply(`Please mention the prefix to use! \n${Info.help}`);
		...
		if (Info.not_allowed.includes(newPref))
			return message.reply(
				`Sorry, but that prefix is not allowed to be used! Please try a different one!`
			);
