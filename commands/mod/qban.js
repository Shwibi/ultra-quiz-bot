	...
	async call(message) {
		...
		if (args.length == 0)
			return message.channel.send(
				`Please provide the ID of the user or mention the user you want to ban!`
			);
		...
		if (args[1] == "unban") {
			if (!previousBanList.includes(userId))
				return message.channel.send(`User does not exist in ban list!`);
			await guildDB
				.updateOne({
					$pull: {
						bannedUsers: userId,
			...
			if (previousBanList.includes(userId))
				return message.channel.send(`User already exists in ban list!`);
			await guildDB
				.updateOne({
					$push: {
						bannedUsers: userId,
