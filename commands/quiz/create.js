 ...
 */
  ...
  async call(message) {
    ...
    message.channel.send(
      `Creating a new quiz with the name **${quizDetails.name}**. Please select the format in which you will provide questions,
          all in one message or one by one? Type \`all\` for all in one message, or type \`one\` for one by one. If you choose
          one by one method, you can end the questions by typing \`end\` instead of your question.
          You have 20 seconds to choose, then this thread will close.
          `
    ).then(
      initialMessage => {
            ...
            message.channel.send(
              ...
              allQuestionsCollector.on("collect", async allQuestionsCollected => {
                this.parseQuestions(allQuestionsCollected.content, async (allQuestions) => {
                  let quizId = await QuizModel.count({});
                  quizId += 1;
  ...
  parseQuestions(allQuestionsContent, callback = (allQuestions) => { }) {
    ...
    questions.forEach(question => {
      ...
      const optsRI = optsR.indexOf("-p" || "-t" || "~");
      const optsEnd = optsRI > 0 ? optsRI : optsR.length;
